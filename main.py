...
cpuGpu = Label(window,  fg="white", bg="#add123", font=("Lucida", 22))

# frame = Frame(window, width=400, height=100,bg="#add123")
def Update():
    global date, datetime , timelabel, dateLabel ,window ,frame ,cpuGpu,psutil,cpufreq,gpus
    date = date.today()
    now = datetime.now()
    ...
    cpuGpu['text']=f"CPU: {psutil.cpu_percent()}%  "
    timelabel.pack()
    dateLabel.pack()
    # frame.pack()
    cpuGpu.pack()
